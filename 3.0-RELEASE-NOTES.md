...
loopback errorhandler is removed and users were told to migrate to the new
strong-error-handler. Its purpose was to enable or disable stack inclusion,
which is no longer needed with strong-error-handler.
```
module.exports = function(options) {
  throw new Error ('expressHandler is no longer coupled with loopback.\
  Please migrate to strong-error-handler');
};
```

Please see [related code change](https://github.com/strongloop/loopback/pull/2411) here.
